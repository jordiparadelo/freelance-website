...
export async function getLeads(options?: StrapiQueryOptions) {
  ...
  const data = options
    ? await getStrapiData(query, {
        options,
        config: { tags: ["leads"], revalidate: 0 },
      })
    : await getStrapiData(query, {
        config: { tags: ["leads"], revalidate: 0 },
      });
...
export async function getLeadByEmail(email: string) {
  ...
  const filteredLead = leads[0];

  return filteredLead as Lead;
}

export async function createNewLead(newLead: NewLeadData) {
  try {
    const baseUrl = getDataUrl();
    ...
    throw new Error(`Error creating new lead ${error}`);
  }
}
