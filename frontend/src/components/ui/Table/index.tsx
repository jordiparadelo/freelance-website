...
const Table = ({ children, className, data, ref }: TableProps) => {
  ...
  return (
    <TableContext.Provider value={true}>
      {data ? (
        <DataTable data={data} className={composedClassName} ref={ref} />
      ) : (
        <NestedTable className={composedClassName} ref={ref}>
          {children}
        </NestedTable>
      )}
...
function NestedTable({ className, children, ref }: TableChild) {
  return (
...
const DataTable = ({
  ...
  className: TableChild["className"];
  ref: TableChild["ref"];
}) => (
  <div className={className} ref={ref}>
    <TableHead>
